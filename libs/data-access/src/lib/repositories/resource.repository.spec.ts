...
describe('ResourceRepository', () => {
  ...
  describe('findAuthorizedMachinesForWorker', () => {
    ...
    it('should return authorized machines for worker using loginCode', async () => {
      hanaService.query.mockResolvedValue(mockMachines);

      // Uses loginCode (U_password), not empID
      ...
      // Verify loginCode is passed correctly
      expect(hanaService.query).toHaveBeenCalledWith(
        expect.any(String),
        ['200', '200', '200']
      );
    });
  ...
  describe('isWorkerAuthorizedForMachine', () => {
    it('should return true when worker is authorized', async () => {
      ...
      // Verify parameters: resCode, loginCode, loginCode
      expect(hanaService.queryOne).toHaveBeenCalledWith(
        expect.any(String),
        ['1001 - BARMAG 1', '200', '200']
      );
    });
